"""Deterministische ID- und Referenzvergabe (Abschnitt 6.4).

Designprinzip des Spikes: Alles, was deterministisch lösbar ist, gehört in
Code. IDs sind deterministisch lösbar, also vergibt sie ausschließlich der
Code – in beiden Varianten, auch nach jeder Korrekturrunde.

Eine bewusste Auslegungsentscheidung, die für die Messung entscheidend ist:
...
umbiegen, wäre die Metrik "kaputte Referenzen" in beiden Varianten immer 0
und der Vergleich wertlos. Der Code besitzt also den ID-Raum (Syntax), das
Modell besitzt die Verknüpfung (Semantik) – und genau diese Verknüpfung
wird gemessen.
"""
...
def repin_identity(resource: dict, expected_id: str, expected_type: str) -> list[str]:
    """Setzt ID und Typ nach einer Korrekturrunde wieder auf den Code-Stand.

    Das Modell darf im Korrekturschritt weder die ID noch den Ressourcentyp
    ändern. Tut es das doch, wird korrigiert und der Vorgang gemeldet.
    """
    notes: list[str] = []
